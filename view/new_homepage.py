...
def display_fitbit_log_table(user_email, user_role, user_project, spreadsheet: Spreadsheet) -> None:
    ...
    st.write("Select period in hours to show the data")

    if "period" not in st.session_state:
        st.session_state.period = 120

    st.session_state.period = st.number_input(
        "Select the period in hours to show the data",
        min_value=1,
        max_value=336,
    ...
    with st.spinner("Loading Fitbit data..."):
        try:
            ...
            tot_cols = [x for x in fitbit_log_df.columns if x.lower().startswith("total")]
            curr_cols = [x for x in fitbit_log_df.columns if x.lower().startswith("current")]
