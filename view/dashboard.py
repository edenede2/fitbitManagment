...
def get_available_watches(user_email, user_role, user_project):
    ...
    if user_role == "Admin":
        ...
        available_watches = project_controller.get_all_projects()
        if not available_watches.empty:
            project_names = available_watches['name'].tolist()
            all_watches = pd.DataFrame()
            for project_name in project_names:
                project_watches = project_controller.get_watches_for_project(project_name)
                all_watches = pd.concat([all_watches, project_watches])
            return all_watches
...
def get_watch_data(watch_name, signal_type, start_date, end_date):
    ...
    """
    # Create a folder path from the project root
    data_dir = Path(os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))) / "data"
    
    # Try to load the complete data file
    try:
        history_file = data_dir / "fitbit_data_complete.csv"
        if history_file.exists():
            data = pl.read_csv(history_file)
            
            # Convert to pandas for compatibility with mitosheet
            data = data.to_pandas()
            
            # Filter by watch name
            data = data[data['name'] == watch_name]
            
            # Convert date columns to datetime
            data['syncDate'] = pd.to_datetime(data['syncDate'], errors='coerce')
            
            # Filter by date range
            data = data[(data['syncDate'] >= pd.Timestamp(start_date)) & 
                        (data['syncDate'] <= pd.Timestamp(end_date))]
...
def display_dashboard(user_email, user_role, user_project):
    ...
    if selected_watch:
        ...
        with tab1:
            ...
            with st.spinner(f"Loading {selected_signal} data..."):
